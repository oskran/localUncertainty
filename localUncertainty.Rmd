...
worldUncertainty <- worldUncertainty %>% select(year, NOR)
```
...
database <- database %>% group_by(paper, county, index) %>% mutate(stdr = sd(uncertaintyPerWord, na.rm = T))

database <- database %>% group_by(paper, county, index) %>% mutate(uncertaintySt = uncertaintyPerWord / stdr) #Uncertainty per word divided by sd
...
database <- database %>%
  ...
  summarise(totalSum = sum(total, na.rm = T), # Total words per month
            ...
            uncertaintyWeighted = mean(uncertaintyWeighted, na.rm = T)) # Mean uncertainty per paper, per paper, per month


# Normalize to mean 100 for St
seriesMeanSt <- database %>% group_by(county, index) %>% summarise(mean(uncertaintyStWeighted, na.rm = T))
...
seriesMean <- database %>% group_by(county) %>% summarise(mean(uncertaintyWeighted, na.rm = T))
...
database <-
  ...
  mutate(rollingMeanSt = c(rep(NA, 9), rollmean(uncertaintyNormSt, 10, align = "right")),
         rollingMean = c(rep(NA, 9), rollmean(uncertaintyNorm, 10, align = "right")))
...
database <- left_join(database, arbeidsledighet[, c("dateMonth", "county", "unemployment")])
...
for (i in seq_along(unique(database$county))) {
  database[database$county == unique(database$county)[i], "unempScaler"] <- Skaler(database[database$county == unique(database$county)[i], "unemployment", ], database[database$county == unique(database$county)[i], "uncertaintyNorm"])
}

# Create series with migration for each county from 2000 to 2013
database <- left_join(database, innflytting)
```
